...
module.exports = Map = cls.Class.extend({
    init: function(filepath) {
        ...
        if (!filepath) {
            // Create default map when no filepath provided
            this.initMap(this.createDefaultMap());
            return;
        }
        ...
        fs.access(filepath, fs.constants.F_OK, (err) => {
            ...
            fs.readFile(filepath, function(err, file) {
                if (err) {
                    ...
                    self.initMap(self.createDefaultMap());
                    return;
                }
                try {
    ...
    getRandomStartingPosition: function() {
        ...
        if (nbAreas === 0) {
            // Return default starting position if no areas defined
            return { x: 50, y: 50 };
        }
