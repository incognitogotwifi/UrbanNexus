...
async function onClientAddFriend(player, otherid) {
    ...
    player.showmessage("You have added %s to your friends list!", [other.name], other);
    if (other.isonline && !other.isfriend(player.id)) {
        // Invite the other side to also add this player as friend
        other.showpm(other.translate("I have added you to my friends list. Do you also want to add me?"),
            {userid:player.id, head:player.head, hat:player.hat, name:player.name, isonline:true},
            {type:"invitation", invitationtype:"friend"});
    }
}

function onClientRemoveFriend(player, otherid) {
    if (!otherid || player.id == otherid)
        return;
    player.removefriend(otherid);
    player.showmessage("You have removed the player from your friends list!");
}

function onClientSaveStatus(player, status, social) {
    // echo("save status:", status, social);
    if (player.statusban)
        return;

    player.status = status;
    player.youtube   = (social && social.indexOf("youtu") >= 0?        social : null);
    player.instagram = (social && social.indexOf("instagram") >= 0?    social : null);
    player.facebook  = (social && social.indexOf("facebook.com") >= 0? social : null);
    player.twitch    = (social && social.indexOf("twitch") >= 0?       social : null);
    player.tiktok    = (social && social.indexOf("tiktok.com") >= 0?   social : null);
    player.save();

    this.onClientLoadProfile(player, player.id);
}
